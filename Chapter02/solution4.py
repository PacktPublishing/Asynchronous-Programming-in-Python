...
from multiprocessing import log_to_stderr, Queue
...
def great_circle_distance(src_dest: Tuple)->Number:
    ...
    logger.info(f'Distance calculated {source['city_ascii']}-{dest['city_ascii']}:{distance}')
    queue.put(f'From: {source['city_ascii']} to: {dest['city_ascii']}, distance: {distance} nm')
    return distance
...
def calculate_path_distance(cities: List)->Number:
    total = 0
    with Pool(2) as p:
        values = p.map(great_circle_distance, cities)
        total = sum(values)
